...
func (w *window) SetWindowLayout(opts *Options) {
	...
	w.MoveWindow(opts, width, height)
}

func (w *window) MoveWindow(opts *Options, width, height int32) {
	log.Println(opts.Centered)
	if opts.Centered {
		w.CenterWindow(opts, width, height)
	} else {
		windows.MoveWindow(w.hwnd, 0, 0, width, height, true)
	}
}

func (w *window) CenterWindow(opts *Options, width, height int32) {
